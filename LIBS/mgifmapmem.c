...
static MGIF_HEADER_ALIGNED_T mgif_header;
...
static void PlayMGFFile(void *file, MGIF_PROC proc,int ypos,char full)
  ...
  hdr = (MGIF_HEADER_T *)file;
  memcpy(mgif_header.ampl_table,hdr->ampl_table,sizeof(hdr->ampl_table));
  mgif_header.frames = uaGetInt(hdr->frames);
  mgif_header.snd_chans = uaGetShort(hdr->snd_chans);
  mgif_header.snd_freq = uaGetInt(hdr->snd_freq);
...
void BigPlayProc(int act,void *data,int csize)
  ...
  switch (act)
     ...
     case MGIF_PAL:memcpy(paleta,data,512);break;
	 case MGIF_SOUND: 
