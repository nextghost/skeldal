...
void pcx_view_draw(int x1,int y1,int x2,int y2,OBJREC *o)
  ...
  picture=*(char **)(oldname=o->userptr);
  oldname+=4;
  newname=o->data;
  if (newname[0]=='*')
     newname++,mirror=1;
  ...
  if (picture==NULL)
     ...
     if (mirror && picture!=NULL) mirror_pcx((word *)picture);
     }
  if (picture==NULL)
     {
     ...
     if (mirror) mirror_pcx((word *)picture);
     }
  bar(x1,y1,x2,y2);
...
void pohyb_moba(EVENT_MSG *msg,void **data)
  ...
  switch (msg->msg)
     ...
     case E_TIMER:if (delay) delay--;
              ...
              x=320;y=0;xs=1;ys=1;
              ch=open_pcx(name,A_8BIT,(char **)&picture);
              if (ch)
                 {
                 ...
                 z=afile(c,9,&s);
                 if (z!=NULL) {
                	 load_pcx(z,s,A_8BIT,(char **)&picture);
                	 free(z);
                	 ch=0;
                 }else {
                	 ch=1;
...
void mob_inv()
  ...
  for(i=0;i<MOBS_INV;i++)
     {
     gui_define(i+10,10,20+i*15,250,10,0,str_line,ls_sorts);gui_on_enter(string_list_sup);
     c_default(moblist[m].inv[i]);
     }
  gui_define(300,5,20,80,20,1,button,"Ok");gui_property(&b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(310,5,45,80,20,1,button,"Zru�it");gui_property(&b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  redraw_desktop();
  escape();
  if (o_aktual->id==300)
...
void spell_affections()
  ...
  for(i=0;i<16;i++)
     {
     gui_define(i+10,10,20+i*12,250,10,0,check_box,aff_list[i]);c_default((m & (1<<i))!=0);
     }
  gui_define(-1,10,230,1,1,0,label,"(*) - u nestvur neimplementov�no");
  gui_define(-1,10,242,1,1,0,label,"Sanctuary bere potvo�e jen 1/2 �ivot� ze z�sahu");
  gui_define(-1,10,254,1,1,0,label,"High Sanc. o�ez�v� z�sah na max 18 �ivot�");
  gui_define(-1,10,266,1,1,0,label,"Energy drain p�id� 1/2 toho co ubere postav�m");
  gui_define(-1,10,278,1,1,0,label,"BLIND - potvora je slepa (senceless)");
  gui_define(-1,10,290,1,1,0,label,"REGENERACE - Ka�d� kolo +HPREG �ivot�");
  gui_define(300,5,20,80,20,1,button,"Ok");gui_property(&b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(310,5,45,80,20,1,button,"Zru�it");gui_property(&b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  redraw_desktop();
  escape();
...
void mob_sound_call(int mob)
  ...
  for(j=0;j<MOB_SOUNDS-1;j++)
     ...
     z=moblist[i].sounds[j];
     y=160+15*j;
     gui_define(-1,5,y,1,1,0,label,sound_ev[j]);
     gui_define(j+20,120,y,120,12,0,input_line,13);
        gui_property(&b2,NULL,NULL,WINCOLOR);
        if (z && enemy_sound!=NULL && (z<=str_count(enemy_sound)))
           set_default(enemy_sound[z-1]);else set_default("");
     gui_define(j+40,245,y,20,12,0,button,"<");
        gui_property(&b1,NULL,NULL,WINCOLOR);gui_on_change(mob_sound_copy);
     }
  gui_define(90,5,30,100,10,2,check_box,"Loop (WALK!)");c_default((moblist[i].vlajky & 0x40)!=0);
...
static void open_dialog_edit_box()
  ...
  gui_define(9,10,20,200,126,0,listbox,dlg_names,RGB555(31,31,31),0);
  gui_property(&b3,NULL,NULL,WINCOLOR);c_default(pgf2name(j));
  gui_define(10,217,40,19,87,0,scroll_bar_v,0,10,1,SCROLLBARCOL);
  gui_property(&b2,NULL,NULL,WINCOLOR);
  gui_define(11,216,20,21,17,0,scroll_button,-1,0,"\x1e");
  gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
...
void edit_mob(int i)
  ...
  gui_define(200,310,35,290,1,0,pcx_view);set_default("_");
  gui_define(205,310,35,290,370,0,symetry);c_default(moblist[i].adjusting[0]);
  gui_define(207,5,20,120,20,1,input_line,20);set_default("");
  gui_define(-1,190,20,100,12,0,label,"Jm�no potvory");
  gui_define(10,300,20,200,12,0,input_line,31);set_default(moblist[i].name);gui_property(def_border(2,WINCOLOR),NULL,NULL,WINCOLOR);
  z=0;b1=def_border(5,WINCOLOR);
  for(y=20;y<380;)
     {
     if (nvlast[z][0])
	{
     	gui_define(-1,5,y,140,12,0,label,nvlast[z]);
     	gui_define(z+20,120,y,60,12,0,input_line,8,-32767,32767,"%6d");set_default(strs(moblist[i].vlastnosti[z]));gui_on_exit(test_int);
  ...
  y+=15;
  gui_define(-1,5,y,140,12,0,label,"�ivel:");
  gui_define(102,120,y,60,12,0,str_line,zivly);c_default(moblist[i].vlastnosti[VLS_MGZIVEL]);gui_on_enter(string_list_sup);gui_property(b1,NULL,NULL,WINCOLOR);y+=15;
  gui_define(-1,5,y,140,12,0,label,"Jm�no grafiky");
  gui_define(100,120,y,60,12,0,input_line,6);set_default(moblist[i].mobs_name);gui_property(b1,NULL,NULL,WINCOLOR);gui_on_exit(check_mobs_name);y+=15;
  gui_define(-1,5,y,140,12,0,label,"Rychlost");
  gui_define(101,130,y,30,12,0,input_line,3,0,128,"%3d");set_default(strs(moblist[i].speed));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);y+=15;
  gui_define(-1,5,y,140,12,0,label,"�ance �t�ku");
  gui_define(103,130,y,30,12,0,input_line,3,0,255,"%3d");set_default(strs(moblist[i].flee_num));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);y+=15;
  gui_define(-1,5,y,140,12,0,label,"Kouzlo");
  gui_define(104,130,y,30,12,0,input_line,3,0,255,"%3d");set_default(strs(moblist[i].casting));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);y+=15;
  gui_define(-1,5,y,140,12,0,label,"Specproc");
  gui_define(105,120,y,60,12,0,str_line,mob_procs);c_default(moblist[i].specproc);gui_on_enter(string_list_sup);gui_property(b1,NULL,NULL,WINCOLOR);y+=15;
  y=260;gui_define(-1,220,248,140,12,0,label,"Po�et anim.");
  for(z=0;z<6;z++)
     {
     gui_define(-1,190,y,50,12,0,label,smery_anim[z]);
     gui_define(z+120,260,y,20,12,0,input_line,2,!z*2,15,"%2d");set_default(strs(moblist[i].anim_counts[z]));gui_on_exit(test_int);
     gui_property(b1,NULL,NULL,WINCOLOR);
     y+=15;
     }
  gui_define(127,285,320,20,12,0,input_line,2,1,15,"%2d");set_default(strs(moblist[i].hit_pos));gui_on_exit(test_int);
     gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(430,190,350,50,10,0,check_box,"Chod�");
  gui_define(440,190,362,50,10,0,check_box,"�to��");
  gui_define(450,190,374,50,10,0,check_box,"Sly��");
  gui_define(460,250,350,100,10,0,check_box,"Jeden na pol��ku");
  gui_define(470,250,362,60,10,0,check_box,"Str��ce");
  gui_define(480,250,374,40,10,0,check_box,"Sebere");
  gui_define(490,250,386,40,10,0,check_box,"Sb�r�");
  gui_define(500,190,386,50,10,0,check_box,"St��l�");
  gui_define(510,20,438,50,10,0,check_box,"Dialog.");c_default(moblist[i].dialog>=0);gui_on_change(open_dialog_edit_box);
  gui_define(-1,190,50,50,12,0,label,"Dohled");
  gui_define(-1,190,65,50,12,0,label,"Dosah");
  gui_define(-1,190,80,50,12,0,label,"Expy:");
  gui_define(-1,190,95,50,12,0,label,"Bonus:");
  gui_define(-1,190,110,50,12,0,label,"Pen�ze:");
  gui_define(160,260,50,30,12,0,input_line,3,0,255,"%3d");set_default(strs(moblist[i].dohled));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(170,260,65,30,12,0,input_line,3,0,255,"%3d");set_default(strs(moblist[i].dosah));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(175,250,80,55,12,0,input_line,6,0,999999,"%6d");set_default(strs(moblist[i].experience));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(176,250,95,55,12,0,input_line,6,0,65535,"%6d");set_default(strs(moblist[i].bonus));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(177,255,110,50,12,0,input_line,6,0,65535,"%5d");set_default(strs(moblist[i].money));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(180,0,0,0,0,0,value_store,4);c_default(0);//cislo_animace
  gui_define(185,0,0,0,0,0,value_store,4);c_default(0);//cislo_pozice
  gui_define(190,0,0,0,0,0,value_store,4);c_default(i);//cislo_i
  b1=def_border(1,0);
  gui_define(210,190,144,80,20,0,button,"Dal�� >>");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(mob_next);
  gui_define(220,190,166,80,20,0,button,"Oto�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(mob_turn);
  gui_define(230,190,188,80,20,0,button,"Test");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(play_mob);
  gui_define(240,84,2,80,20,2,button,"Zru�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(250,2,2,80,20,2,button,"Ok");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(260,166,2,80,20,2,button,"Vymazat");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(mob_ask_delete);
  gui_define(270,2,24,80,20,2,button,"Zvuky");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(mob_sound);
  gui_define(280,84,24,80,20,2,button,"Invent��");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(mob_inv);
  gui_define(290,166,24,80,20,2,button,"Aff");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(spell_affections);
  gui_define(-1,190,220,1,1,0,label,"Pals:");
  gui_define(520,230,220,30,12,0,input_line,3,-127,127,"%3d");set_default(strs(moblist[i].paletts_count));gui_on_exit(test_int);gui_property(b1,NULL,NULL,WINCOLOR);
  for(y=0;y<6*16;y++) {gui_define(300+y,0,0,0,0,0,value_store,4);c_default(moblist[i].adjusting[y]);}
  gui_define(630,0,0,0,0,0,value_store,1);c_default(0);//IN_BATTLE;
  gui_define(640,190,400,50,10,0,check_box,"Pr�choz�");
  gui_define(650,190,412,50,10,0,check_box,"C�t�");
  gui_define(660,190,424,100,10,0,check_box,"Astral (wind)");
  gui_define(670,300,400,100,10,0,check_box,"Reload mob");
  gui_define(680,300,412,100,10,0,check_box,"Kouzl�");
  gui_define(700,0,0,0,0,0,value_store,4);c_default(moblist[i].vlastnosti[VLS_KOUZLA]);
  ...
  if (z==260)
     ...
     for(j=0;j<maplen;j++) if ((mob_map[j] & 0x3fff)==i)
        ...
        redraw=1;
        jdi_na_sektor(j);
        if (msg_box("Potvora je v map�. Vymazat?",'\x2',"Chce� vymazat tuto potvoru a v�echny dal��?","Ano","Ne",NULL)==2) return;
        break;
        }
...
void enemy_window()
  ...
  if (find_window(enemy_win)==NULL)
     ...
     gui_define(29,5,20,170,190,0,listbox,enemy_list,RGB555(31,31,0),4);c_default(0);
        o_end->autoresizex=1;
        o_end->autoresizey=1;
     b1=def_border(3,WINCOLOR);
     gui_define(30,5,38,15,156,1,scroll_bar_v,0,10,1,SCROLLBARCOL);
        gui_property(b1,NULL,NULL,WINCOLOR);
        o_end->autoresizey=1;
     gui_define(31,3,20,19,15,1,scroll_button,-1,0,"\x1e");
        gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
     gui_define(32,3,68,19,15,2,scroll_button,1,10,"\x1f");
        gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
     gui_define(40,5,5,60,20,3,button,"Vymazat");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(vymazat_moba);
     gui_define(50,5,30,60,20,3,button,"Um�stit");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(umistit_moba);
     gui_define(60,70,5,60,20,3,button,"Novou");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(new_mob);
     gui_define(70,70,30,60,20,3,button,"Opravit");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(edit_mob_event);
