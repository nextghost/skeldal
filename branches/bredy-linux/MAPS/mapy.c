  ...
  for (i=1;i<maplen;i++)
     ...
     if (x>=0 && x<mapx && y>=0 && y<mapy)
        ...
        if (!j) bar(x,y,x+M_ZOOM-1,y+M_ZOOM-1);
          ...
          for (q=0;q<4;q++)
        	  prepinac(x,y,q,&(mapa.sidedef[i][q]),(multi_actions[(i<<2)+q]!=NULL));
           }
        }
     }
  }
...
void edit_map_bar(int x1,int y1,int x2,int y2)
  ...
  if (isKeyCTRLPressed()) unselect_map();
  swaps(&x1,&y1,&x2,&y2);
  for(y=y1;y<y2;y++)
     for(x=x1;x<x2;x++)
       {
...
void create_map_win(int xp,...)
  ...
  if (find_window(map_win)==NULL)
     ...
     gui_on_change(close_with_tool);
     gui_define(10,2,20,473,408,0,workspace);
     gui_property(def_border(1,0),NULL,NULL,WINCOLOR);
     o_end->autoresizex=1;o_end->autoresizey=1;
     ctl=def_border(3,WINCOLOR);
     waktual->minsizx=140;
     waktual->minsizy=90;
     gui_define(20,3,42,17,365,1,scroll_bar_v,-100,100,(59*8)/M_ZOOM,SCROLLBARCOL);
     gui_property(ctl,NULL,NULL,WINCOLOR);c_default(ymap_offs);gui_on_change(scroll_workspace);
     o_end->autoresizey=1;
     gui_define(21,1,20,21,17,1,scroll_button,-2,-100,"\x1e");
     gui_property(NULL,icones,&cl,WINCOLOR);gui_on_change(scroll_support);
     gui_define(22,1,22,21,17,2,scroll_button,2,100,"\x1f");
     gui_property(NULL,icones,&cl,WINCOLOR);gui_on_change(scroll_support);
     gui_define(30,26,3,352,15,3,scroll_bar_h,-100,100,(59*8)/M_ZOOM,SCROLLBARCOL);
     gui_property(ctl,NULL,NULL,WINCOLOR);c_default(xmap_offs);gui_on_change(scroll_workspace);
     o_end->autoresizex=1;
     gui_define(31,96,1,21,19,2,scroll_button,2,100,"\x1c");
     gui_property(NULL,icones,&cl,WINCOLOR);gui_on_change(scroll_support);
     gui_define(32,1,1,21,19,3,scroll_button,-2,-100,"\x1d");
     gui_property(NULL,icones,&cl,WINCOLOR);gui_on_change(scroll_support);
     gui_define(40,60,1,30,19,2,button,"/");gui_property(NULL,icones,&cl,WINCOLOR);
     gui_on_enter(layer_show);gui_on_exit(layer_hide);gui_on_change(layer_plus);
     gui_define(50,28,1,30,19,2,button,"\\");gui_property(NULL,icones,&cl,WINCOLOR);
     gui_on_enter(layer_show);gui_on_exit(layer_hide);gui_on_change(layer_minus);
     gui_define(60,1,1,19,19,2,resizer);
     xpp=&xp;
     xpp++;
    ...
    if (find_window(tool_bar)==NULL)
     ...
     gui_define(20,1,55,40,34,0,toggle_button,"\xC");gui_property(NULL,icones,&icone_color,WINCOLOR);
      gui_on_change(tool_select);c_default(tool_sel==20);
     gui_define(30,42,20,40,34,0,toggle_button,"\xD");gui_property(NULL,icones,&icone_color,WINCOLOR);
      gui_on_change(tool_select);c_default(tool_sel==30);
     gui_define(40,42,55,40,34,0,toggle_button,"\xE");gui_property(NULL,icones,&icone_color,WINCOLOR);
      gui_on_change(tool_select);c_default(tool_sel==40);
     gui_define(10,1,20,40,34,0,toggle_button,"\xB");gui_property(NULL,icones,&icone_color,WINCOLOR);
      gui_on_change(tool_select);c_default(tool_sel==10);
     }
