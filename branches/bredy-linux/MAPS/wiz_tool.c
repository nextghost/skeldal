...
void create_door(void)
  ...
  gui_define(CANCEL_BUTT,10,5,80,20,2,button,"Zru�it");gui_property(&b1,NULL,NULL,WINCOLOR);
    gui_on_change(gui_terminate);
  gui_define(OK_BUTT,190,5,80,20,2,button,"Ok");gui_property(&b1,NULL,NULL,WINCOLOR);
    gui_on_change(build_door);
  gui_define(10,5,20,192,35,0,label,"Animace 1:   Po�et okenek:");gui_property(&b3,NULL,NULL,WINCOLOR);
  gui_define(20,10,35,90,12,0,str_line,side_names);gui_property(&b2,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  gui_define(30,120,35,50,12,0,input_line,10,1,16,"%6d");gui_property(&b2,NULL,NULL,WINCOLOR);
   set_default(strs(1));gui_on_exit(test_int);
  gui_define(40,5,60,192,35,0,label,"Animace 2:   Po�et okenek:");gui_property(&b3,NULL,NULL,WINCOLOR);
  gui_define(50,10,75,90,12,0,str_line,side_names);gui_property(&b2,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  gui_define(60,120,75,50,12,0,input_line,10,1,16,"%6d");gui_property(&b2,NULL,NULL,WINCOLOR);
   set_default(strs(1));gui_on_exit(test_int);
  gui_define(-1,220,20,100,10,0,label,"Vytvo�it dve�e na:");
  gui_define(70,220,35,100,60,0,radio_butts,5,
                             "Severu",
                             "V�chodu",
                             "Jihu",
                             "Z�padu",
                             "Jako otev�rac� sektor");c_default(0);
  gui_define(-1,5,100,390,160,0,label,"Detaily:");gui_property(&b3,NULL,NULL,WINCOLOR);
  gui_define(-1,200,110,0,140,0,label,""); gui_property(&b2,NULL,NULL,WINCOLOR);
  gui_define(-1,10,112,180,12,1,label,"Funkce dve��:");
  gui_define(80,10,124,160,40,1,radio_butts,3,"Lze jenom otev��t",
                                          "Lze jenom zav��t",
                                          "Lze otev��t i zav��t");c_default(2);
  gui_define(90,10,170,170,12,1,check_box,"D�lkov� ovl�d�n�");c_default(0);gui_on_change(remote_acces);
  gui_define(-1,10,112,190,12,0,label,"Kdy� jsou dve�e zav�en�");
  gui_define(140,15,124,160,10,0,check_box,"Nepr�choz� hr��em");c_default(1);
  gui_define(110,15,136,160,10,0,check_box,"Nepr�choz� nest�rou");c_default(1);
  gui_define(120,15,148,160,10,0,check_box,"Nelze prohodit v�c");c_default(1);
  gui_define(130,15,160,160,10,0,check_box,"Zvukot�sn�");c_default(1);
  gui_define(100,15,172,160,10,0,check_box,"Nepr�hledn�");c_default(1);
  gui_define(-1,10,190,190,12,0,label,"Kdy� jsou dve�e otev�en�");
  gui_define(190,15,202,160,10,0,check_box,"Nepr�choz� hr��em");c_default(0);
  gui_define(160,15,214,160,10,0,check_box,"Nepr�choz� nest�rou");c_default(0);
  gui_define(170,15,226,160,10,0,check_box,"Nelze prohodit v�c");c_default(0);
  gui_define(180,15,238,160,10,0,check_box,"Zvukot�sn�");c_default(0);
  gui_define(150,15,250,160,10,0,check_box,"Nepr�hledn�");c_default(0);
  gui_define(200,10,190,160,12,1,check_box,"Otev��t po startu");c_default(0);gui_on_change(remote_acces);
  gui_define(-1,10,210,160,12,1,label,"Lclip:");gui_property(&b3,NULL,NULL,WINCOLOR);
  gui_define(210,10,210,50,12,1,input_line,10,1,75,"%6d");gui_property(&b2,NULL,NULL,WINCOLOR);
   set_default(strs(1));gui_on_exit(test_int);
  gui_define(300,0,0,0,0,0,value_store,4);c_default(i);
  redraw_window();
  escape();
...
void crt_oblouky()
{
  ...
  gui_define(-1,5,20,70,10,0,label,"Zvol oblouk:");
  gui_define(10,5,32,130,12,0,str_line,oblouky);gui_property(&b2,NULL,NULL,WINCOLOR);c_default(0);
  gui_on_enter(string_list_sup);
  gui_define(20,5,45,100,10,0,check_box,"V rozich pouze");c_default(v_rozich);
  gui_define(30,5,5,60,20,3,button,"Ok");gui_on_change(gui_terminate);
  gui_define(40,5,5,60,20,2,button,"Zru�it");gui_on_change(gui_terminate);
  redraw_window();
  escape();
  if (o_aktual->id==30)
     {
      ...
      if (i==1) {
       if (!strcmp(text,verify))
          ...
          if (j==1) set_password(text);else set_data_password(text);
    	   ...
    	   msg_box("Nejsou schodn�",1,"Kontrola nesouhlasila. Heslo nebylo zm�n�no!","Ok",NULL);
       }
      }
      }
  }
...
void preference()
  ...
  gui_define(-1,10,25,150,12,0,label,"Pozad� sever:");
  gui_define(-1,10,45,150,12,0,label,"Pozad� v�chod:");
  gui_define(-1,10,65,150,12,0,label,"Pozad� jih:");
  gui_define(-1,10,85,150,12,0,label,"Pozad� zapad:");
  gui_define(-1,10,105,150,12,0,label,"Barva �erven�:");
  gui_define(-1,10,125,150,12,0,label,"Barva zelena:");
  gui_define(-1,10,145,150,12,0,label,"Barva modra:");
  gui_define(-1,10,165,150,12,0,label,"Start:");
  gui_define(-1,10,185,150,12,0,label,"Start sm�r:");
  gui_define(-1,10,25,150,12,3,label,"Jm�no urovn�:");
  for(i=0;i<4;i++)
     {
     gui_define(10+10*i,150,25+20*i,100,15,0,input_line,12);set_default(mglob.back_fnames[i]);
     gui_property(&b3,NULL,NULL,WINCOLOR);
     }
  gui_define(50,150,105,50,15,0,input_line,5,0,255,"%5d");set_default(strs(mglob.fade_r));
     gui_property(&b3,NULL,NULL,WINCOLOR);gui_on_exit(test_int);
  gui_define(60,150,125,50,15,0,input_line,5,0,255,"%5d");set_default(strs(mglob.fade_g));
     gui_property(&b3,NULL,NULL,WINCOLOR);gui_on_exit(test_int);
  gui_define(70,150,145,50,15,0,input_line,5,0,255,"%5d");set_default(strs(mglob.fade_b));
     gui_property(&b3,NULL,NULL,WINCOLOR);gui_on_exit(test_int);
  gui_define(80,150,165,50,15,0,input_line,5,1,maplen,"%5d");set_default(strs(mglob.start_sector));
     gui_property(&b3,NULL,NULL,WINCOLOR);gui_on_exit(test_int);
  gui_define(90,150,185,100,60,0,radio_butts,4,
                             "Sever",
                             "V�chod",
                             "Jih",
                             "Z�pad");c_default(mglob.direction);
  gui_define(95,210,105,100,60,0,radio_butts,5,
                             "Normaln�",
                             "Sopka (��r)",
                             "Ledov� j.(mr�z)",
                             "Pod vodou",
                             "M�sto");c_default(mglob.map_effector);
  gui_define(96,210,165,100,10,0,check_box,"Lok�ln� nestv�ry");c_default(mglob.local_monsters);gui_on_change(local_monsters_warning);
  gui_define(97,210,180,100,10,0,check_box,"Autom.st�nuj p&s");c_default(mglob.map_autofadefc);
  gui_define(CANCEL_BUTT,10,10,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);
  gui_define(OK_BUTT,10,35,80,20,2,button,"Ok");gui_on_change(gui_terminate);
  gui_define(110,10,60,100,20,2,button,"Zabezpe�en�>>");gui_on_change(zabezpecit);
  gui_define(100,10,10,200,11,3,input_line,29);set_default(mglob.mapname);
  gui_property(&b3,NULL,NULL,WINCOLOR);
  redraw_window();
  escape();
...
void open_blok_window()
  ...
  gui_define(50,10,20,100,10,0,check_box,"Na Okraji");c_default(0);
  gui_define(60,10,32,100,10,0,check_box,"Vn�");c_default(0);
  gui_define(10,10,44,100,10,0,check_box,"Na severu");c_default(1);
  gui_define(20,10,56,100,10,0,check_box,"Na v�chod�");c_default(1);
  gui_define(30,10,68,100,10,0,check_box,"Na jihu");c_default(1);
  gui_define(40,10,80,100,10,0,check_box,"Na z�pad�");c_default(1);
  gui_define(-1,10,100,1,1,0,label,"Zap Vyp");
  gui_define(100,40,112,150,10,0,check_box,"Pr�hlednost");c_default(0);
  gui_define(110,40,124,150,10,0,check_box,"Dva druhy st�n");c_default(0);
  gui_define(120,40,136,150,10,0,check_box,"Tajn� st�ny");c_default(0);
  gui_define(130,40,148,150,10,0,check_box,"Prim. viditeln�");c_default(0);
  gui_define(140,40,160,150,10,0,check_box,"Prim. animace");c_default(0);
  gui_define(200,40,172,30,30,0,radio_butts,3,"-","\x4","\x6");gui_property(NULL,icones,NULL,WINCOLOR);c_default(0);
  gui_define(101,25,112,10,10,0,check_box,"");c_default(0);
  gui_define(111,25,124,10,10,0,check_box,"");c_default(0);
  gui_define(121,25,136,10,10,0,check_box,"");c_default(0);
  gui_define(131,25,148,10,10,0,check_box,"");c_default(0);
  gui_define(141,25,160,10,10,0,check_box,"");c_default(0);
  gui_define(151,25,172,10,10,0,check_box,"");c_default(0);
  gui_define(OK_BUTT,5,5,80,20,2,button,"Ok");gui_on_change(gui_terminate);
  redraw_window();
  escape();
...
static void schody(void)
  ...
  gui_define(CANCEL_BUTT,10,5,80,20,2,button,"Zru�it");gui_property(&b1,NULL,NULL,WINCOLOR);
    gui_on_change(gui_terminate);
  gui_define(OK_BUTT,190,5,80,20,2,button,"Ok");gui_property(&b1,NULL,NULL,WINCOLOR);
    gui_on_change(build_stairs);
  gui_define(10,5,20,192,35,0,label,"Tady:     Po�et okenek:");gui_property(&b3,NULL,NULL,WINCOLOR);
  gui_define(20,10,35,90,12,0,str_line,side_names);gui_property(&b2,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  gui_define(30,120,35,50,12,0,input_line,10,1,16,"%6d");gui_property(&b2,NULL,NULL,WINCOLOR);
   set_default(strs(1));gui_on_exit(test_int);
  gui_define(40,5,60,192,35,0,label,"Tam:      Po�et okenek:");gui_property(&b3,NULL,NULL,WINCOLOR);
  gui_define(50,10,75,90,12,0,str_line,side_names);gui_property(&b2,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  gui_define(60,120,75,50,12,0,input_line,10,1,16,"%6d");gui_property(&b2,NULL,NULL,WINCOLOR);
   set_default(strs(1));gui_on_exit(test_int);
  gui_define(-1,220,20,100,10,0,label,"Vytvo�it schody na:");
  gui_define(70,220,35,100,60,0,radio_butts,4,
                             "Severu",
                             "V�chodu",
                             "Jihu",
                             "Z�padu");c_default(0);
  gui_define(-1,5,100,390,160,0,label,"Detaily:");gui_property(&b3,NULL,NULL,WINCOLOR);
  gui_define(-1,200,110,0,140,0,label,""); gui_property(&b2,NULL,NULL,WINCOLOR);
  gui_define(-1,10,112,180,12,1,label,"Schody povedou:");
  gui_define(80,10,124,160,40,1,radio_butts,3,"Nahoru",
                                          "Dolu",
                                          "Na sektor");c_default(2);
  gui_define(-1,10,112,180,12,0,label,"��slo sektoru:");
  gui_define(90,130,112,50,12,0,input_line,10,1,maplen,"%6d");gui_property(&b2,NULL,NULL,WINCOLOR);
   set_default(strs(1));gui_on_exit(test_int);
  gui_define(100,10,130,180,10,0,check_box,"P��m� spojen�");c_default(0);
  gui_define(300,0,0,0,0,0,value_store,4);c_default(i);
  redraw_window();
  escape();
  close_window(waktual);
...
static void vyber(void)
  ...
  gui_define(CANCEL_BUTT,10,5,80,20,2,button,"Zru�it");gui_property(&b1,NULL,NULL,WINCOLOR);
    gui_on_change(gui_terminate);
  gui_define(OK_BUTT,100,5,80,20,2,button,"Ok");gui_property(&b1,NULL,NULL,WINCOLOR);
    gui_on_change(gui_terminate);
  gui_define(-1,10,20,60,10,0,label,"Vyber st�nu kterou chce� ozna�it:");
  gui_define(400,10,40,100,10,0,check_box,"Severn�");c_default(0);
  gui_define(410,10,55,100,10,0,check_box,"V�chodn�");c_default(0);
  gui_define(420,10,70,100,10,0,check_box,"Ji�n�");c_default(0);
  gui_define(430,10,85,100,10,0,check_box,"Z�padn�");c_default(0);
  gui_define(440,10,100,200,10,0,check_box,"Podle prim�rn� st�ny");c_default(0);
  gui_define(20,10,115,200,12,0,str_line,side_names);gui_property(&b2,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  gui_define(450,10,130,200,10,0,check_box,"Podle sekund�rn� st�ny");c_default(0);
  gui_define(30,10,145,200,12,0,str_line,side_names);gui_property(&b2,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  redraw_window();
  escape();
  if (o_aktual->id==OK_BUTT)
...
void open_wiz_tool(void)
  ...
  gui_on_change(close_wiz_tool);
  gui_define(10,10,25,100,19,0,button2,"Dve�e");gui_on_change(create_door);
  gui_define(10,10,45,100,19,0,button2,"Oblouky");gui_on_change(crt_oblouky);
  gui_define(10,10,65,100,19,0,button2,"Schody");gui_on_change(schody);
  gui_define(10,10,85,100,19,0,button2,"Globaln�");gui_on_change(preference);
  gui_define(10,10,105,100,19,0,button2,"Bloky");gui_on_change(open_blok_window);
  gui_define(10,10,125,100,19,0,button2,"Multiakce");gui_on_change(open_multiaction_window);
  gui_define(10,10,145,100,19,0,button2,"P�edvolba");gui_on_change(predvolba);
  gui_define(10,10,165,100,19,0,button2,"V�b�r");gui_on_change(vyber);
  }
  else
  {
...
opakuj:
  ...
  set_window_modal();
  gui_on_change(gui_terminate);
  gui_define(9,10,20,120,420,0,listbox,l,RGB555(31,31,0),0);c_default(0);
  gui_on_change(gui_terminate);
  o_end->autoresizex=1;
  o_end->autoresizey=1;
  gui_define(10,3,42,21,360,1,scroll_bar_v,0,10,1,SCROLLBARCOL);
  gui_property(NULL,NULL,NULL,WINCOLOR);
  o_end->autoresizey=1;
  gui_define(11,3,20,21,17,1,scroll_button,-1,0,"\x1e");
  gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
  gui_define(12,3,22,21,17,2,scroll_button,1,10,"\x1f");
  gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
...
void add_ma_flags(int start,int x,int y,TMULTI_ACTION *p)
  {
  gui_define(-1,x,y,1,1,0,label,"Podm�nky");
  gui_define(start+0,x+25,y+15,140,10,0,check_box,"Cancel program");c_default(p->general.cancel);
  gui_define(start+1,x+25,y+27,140,10,0,check_box,"Jedenkr�t");c_default(p->general.once);
  gui_define(start+2,x+25,y+39,140,10,0,check_box,"�sp��n� pr�chod");c_default(p->general.ps);
  gui_define(start+3,x+25,y+51,140,10,0,check_box,"Ne�sp��n� pr�chod");c_default(p->general.pf);
  gui_define(start+4,x+25,y+63,140,10,0,check_box,"�sp��n� dotyk");c_default(p->general.ts);
  gui_define(start+5,x+25,y+75,140,10,0,check_box,"Ne�sp��n� dotyk");c_default(p->general.tf);
  gui_define(start+6,x+25,y+87,140,10,0,check_box,"Informace o z�mku");c_default(p->general.li);
  gui_define(start+7,x+25,y+99,140,10,0,check_box,"P�ed odchodem");c_default(p->general.ul);
  gui_define(start+8,x+25,y+111,140,10,0,check_box,"P�i p��chodu akce");c_default(p->general.ia);
  gui_define(start+9,x+25,y+123,140,10,0,check_box,"P�i startu levelu");c_default(p->general.sp);
  gui_define(start+10,x+25,y+135,140,10,0,check_box,"P�i uzav�en� dve��");c_default(p->general.as);
  gui_define(start+11,x+25,y+147,140,10,0,check_box,"Ka�de anim.pol��ko");c_default(p->general.am);
  gui_define(start+12,x+25,y+159,140,10,0,check_box,"Ka�de druh� a.p.");c_default(p->general.a2);
  gui_define(start+13,x+25,y+171,140,10,0,check_box,"P�i proveden� akce");c_default(p->general.pa);
  gui_define(start+14,x+25,y+183,140,10,0,check_box,"�sp��n� SpecProc");c_default(p->general.us);
  gui_define(start+15,x+25,y+195,140,10,0,check_box,"P�i otev�en� dve��");c_default(p->general.od);
  gui_define(start+16,x+25,y+207,140,10,0,check_box,"Ud�lost v�klenku");c_default(p->general.vy);
  }
...
void tma_sound_update()
  ...
  if ((rff=find_chunk(riff,WAV_FMT))!=NULL && (rff2=find_chunk(riff,WAV_DATA))!=NULL)
     ...
     if (fmt.wav_mode==1 && fmt.chans==1)
        ...
        name_conv(c);
        set_value(0,20,c);
        sprintf(c,"%6ld",fmt.freq);set_value(0,30,c);
        sprintf(c,"%6ld",size);set_value(0,40,c);
        sprintf(c,"%6ld",size);set_value(0,50,c);
...
void tma_sound(TMULTI_ACTION *q)
  ...
  gui_define(9,10,20,200,126,0,listbox,list,RGB555(31,31,31),0);
  gui_property(&b3,NULL,NULL,WINCOLOR);c_default(0);gui_on_change(tma_sound_preview);
  ...
  gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
  gui_define(-1,10,160,100,11,0,label,"Jm�no:");
  gui_define(-1,10,175,100,11,0,label,"freq[Hz]:");
  gui_define(-1,10,190,100,11,0,label,"StartLoop:");
  gui_define(-1,10,205,100,11,0,label,"EndLoop:");
  gui_define(-1,10,220,100,11,0,label,"SmpOffset:");
  gui_define(-1,10,235,100,11,0,label,"Hlasitost:");
  gui_define(-1,10,250,100,11,0,label,"Stopa:");
  gui_define(20,90,160,110,11,0,input_line,12);
  gui_property(&b3,NULL,NULL,WINCOLOR);set_default(q->sound.filename);
  gui_define(30,90,175,70,11,0,input_line,7,4000,88000,"%6d");gui_on_exit(test_int);
  gui_property(&b2,NULL,NULL,WINCOLOR);set_default(strs(q->sound.freq));
  gui_define(40,90,190,70,11,0,input_line,7,0,0x7fffffff,"%6d");gui_on_exit(test_int);
  gui_property(&b2,NULL,NULL,WINCOLOR);set_default(strs(q->sound.start_loop));
  gui_define(50,90,205,70,11,0,input_line,7,0,0x7fffffff,"%6d");gui_on_exit(test_int);
  gui_property(&b2,NULL,NULL,WINCOLOR);set_default(strs(q->sound.end_loop));
  gui_define(60,90,220,70,11,0,input_line,7,0,0x7fffffff,"%6d");gui_on_exit(test_int);
  gui_property(&b2,NULL,NULL,WINCOLOR);set_default(strs(q->sound.offset));
  gui_define(70,90,235,70,11,0,input_line,7,0,100,"%6d");gui_on_exit(test_int);
  gui_property(&b2,NULL,NULL,WINCOLOR);set_default(strs(q->sound.volume));
  gui_define(80,90,250,70,11,0,input_line,7,0,100,"%6d");gui_on_exit(test_int);
  gui_property(&b2,NULL,NULL,WINCOLOR);set_default(strs(q->sound.soundid));
  gui_define(100,20,265,100,11,0,check_box,"16Bit");c_default(q->sound.bit16 & 1);
  gui_define(110,20,277,100,11,0,check_box,"Zaka� p�i otev�r�n�");c_default((q->sound.bit16 & 2)>>1);
  gui_define(120,20,290,100,11,0,check_box,"Zaka� p�i zav�r�n�");c_default((q->sound.bit16 & 4)>>2);
  gui_define(130,20,302,100,11,0,check_box,"N�hodn� stereo");c_default((q->sound.bit16 & 8)>>3);
  gui_define(290,5,80,100,11,1,check_box,"Preview");c_default(preview);
  gui_define(295,5,92,100,11,1,check_box,"AutoUpdate");c_default(autoupdate);gui_on_change(tma_sound_autoupdate);
  gui_define(300,5,20,100,20,1,button,"Ok");gui_property(&b1,NULL,NULL,WINCOLOR);
   gui_on_change(gui_terminate);
  gui_define(310,5,45,100,20,1,button,"Zrusit");gui_property(&b1,NULL,NULL,WINCOLOR);
   gui_on_change(gui_terminate);
  gui_define(320,5,160,100,20,1,button,"<< Update");gui_property(&b1,NULL,NULL,WINCOLOR);
   gui_on_change(tma_sound_update);
  gui_define(330,5,135,45,20,1,button,"");gui_property(&b1,NULL,NULL,WINCOLOR);
   gui_on_change(tma_sound_test);
  gui_define(340,60,135,45,20,1,button,"");gui_property(&b1,NULL,NULL,WINCOLOR);
   gui_on_change(tma_sound_stop);
...
void tma_fireball(TMULTI_ACTION *p)
  ...
  gui_define(-1,5,20,1,1,0,label,"Vyst�elen� v�c:");
  gui_define(-1,5,35,1,1,0,label,"Xpoz <-63,63>:");
  gui_define(-1,5,50,1,1,0,label,"Ypoz <0,499>:");
  gui_define(-1,5,65,1,1,0,label,"Zpoz <0,319>:");
  gui_define(-1,5,80,1,1,0,label,"Rychlost:<1,128>");
  gui_define(-1,5,100,120,120,1,show_3dbox);
  add_ma_flags(100,5,100,p);
  gui_define(10,150,20,200,12,0,str_line,ls_sorts);gui_property(b1,NULL,NULL,WINCOLOR);
  c_default (p->fireball.item-1);gui_on_enter(string_list_sup);
  gui_define(20,150,35,60,12,0,input_line,3,-63,63,"%3d");gui_property(b1,NULL,NULL,WINCOLOR);
  set_default(strs(p->fireball.xpos));gui_on_exit(test_int);
  gui_define(30,150,50,60,12,0,input_line,3,0,499,"%3d");gui_property(b1,NULL,NULL,WINCOLOR);
  set_default(strs(p->fireball.ypos));gui_on_exit(test_int);
  gui_define(40,150,65,60,12,0,input_line,3,0,319,"%3d");gui_property(b1,NULL,NULL,WINCOLOR);
  set_default(strs(p->fireball.zpos));gui_on_exit(test_int);
  gui_define(50,150,80,60,12,0,input_line,3,0,319,"%3d");gui_property(b1,NULL,NULL,WINCOLOR);
  set_default(strs(p->fireball.speed));gui_on_exit(test_int);
  b1=def_border(1,0);
  gui_define(60,10,10,80,20,2,button,"Zru�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(70,10,35,80,20,2,button,"Ok");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  redraw_window();
  escape();
  if (o_aktual->id==70)
     {
...
void tma_send(TMULTI_ACTION *q)
  ...
  gui_define(-1,5,22,1,1,0,label,"Sektor:");
  b1=def_border(5,WINCOLOR);
  gui_define(10,80,20,60,12,0,input_line,5,0,maplen,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->send_a.sector));
  gui_define(-1,5,37,1,1,0,label,"St�na:");
  gui_define(20,80,35,100,12,0,str_line,steny2);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->send_a.side);
  gui_define(-1,5,52,1,1,0,label,"Akce:");
  gui_define(30,80,50,150,12,0,str_line,actions);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->send_a.s_action);
  gui_define(-1,5,67,1,1,0,label,"Zpo�d�n�:");
  gui_define(40,80,65,60,12,0,input_line,5,0,255,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->send_a.delay));
  gui_define(110,10,90,200,10,0,check_box,"Zm�na automapingu");
  gui_define(120,10,102,200,10,0,check_box,"Zm�na pr�chodnosti(hr��)");
  gui_define(130,10,114,200,10,0,check_box,"Zm�na pr�chodnosti(nestv�ra)");
  gui_define(140,10,126,200,10,0,check_box,"Zm�na pr�chodnosti(v�c)");
  gui_define(150,10,138,200,10,0,check_box,"Zm�na pr�chodnosti(zvuk)");
  add_ma_flags(200,220,20,q);
  fill_bit_fields(0,110,q->send_a.change_bits,5);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
  redraw_window();
  escape();
  if (o_aktual->id==OK_BUTT)
     {
...
void tma_loadlev(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Sektor:");
  gui_define(20,80,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->loadlev.start_pos));
  gui_define(-1,5,37,1,1,0,label,"Sm�r:");
  gui_define(30,80,35,100,12,0,str_line,steny2);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->loadlev.dir);
  gui_define(-1,5,52,1,1,0,label,"Jm�no:");
  gui_define(10,80,50,100,12,0,input_line,12);gui_property(b1,NULL,NULL,WINCOLOR);set_default(q->loadlev.name);
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_play_anim(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,20,1,1,0,label,"Jm�no:");
  gui_define(10,80,22,100,12,0,input_line,12);gui_property(b1,NULL,NULL,WINCOLOR);set_default(q->loadlev.name);
  gui_define(20,20,40,100,10,0,check_box,"cls");c_default(q->loadlev.dir);
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_write_book(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,20,1,1,0,label,"Kniha:");
  gui_define(10,80,35,100,12,0,input_line,12);gui_property(b1,NULL,NULL,WINCOLOR);set_default(q->loadlev.name);
  gui_define(-1,5,50,1,1,0,label,"��slo odstavce:");
  gui_define(20,80,65,65,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->loadlev.start_pos));
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
...
void tma_swapsectors(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Sektor1:");
  gui_define(20,80,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->swaps.sector1));
  gui_define(-1,5,37,1,1,0,label,"Sektor2:");
  gui_define(30,80,35,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->swaps.sector2));
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_experience(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Zku�enost:");
  gui_define(20,100,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm1));
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_cancel_action(TMULTI_ACTION *q)
  ...
  gui_define(-1,5,22,1,1,0,label,"Sektor:");
  gui_define(20,80,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->cactn.sector));
  gui_define(-1,5,37,1,1,0,label,"Sm�r:");
  gui_define(30,80,35,100,12,0,str_line,steny2);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->cactn.dir);
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_sharema(TMULTI_ACTION *q)
  ...
  gui_define(-1,5,37,1,1,0,label,"Sm�r:");
  gui_define(30,80,35,100,12,0,str_line,steny2);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->twop.parm2);
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
  gui_define(-1,5,100,10,10,0,label,"Spust� MA program na jin�m");
  gui_define(-1,5,110,10,10,0,label,"m�st�, tak jako by byl");
  gui_define(-1,5,120,10,10,0,label,"definov�n zde.");
  gui_define(-1,5,140,10,10,0,label,"Vyu�it�:");
  gui_define(-1,5,150,10,10,0,label,"Sd�len� jednoho program");
  gui_define(-1,5,160,10,10,0,label,"Zm�na v m�st� sd�leni se pak");
  gui_define(-1,5,170,10,10,0,label,"projev� na v�ech m�stech, kter�");
  gui_define(-1,5,180,10,10,0,label,"program sd�lej�.");
  redraw_window();
  escape();
  if (o_aktual->id==OK_BUTT)
     {
...
void tma_teleport_group(TMULTI_ACTION *q)
  ...
  gui_define(-1,5,37,1,1,0,label,"Sm�r:");
  gui_define(30,80,35,100,12,0,str_line,steny2);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->twop.parm2 & 3);
  gui_define(40,80,59,100,10,0,check_box,"Povol efekt");c_default(q->twop.parm2>>7);
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_lock(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"��slo kl��e:");
  gui_define(10,120,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->lock.key_id));
  gui_define(-1,5,37,1,1,0,label,"Urove�:");
  gui_define(20,120,35,60,12,0,input_line,5,-1,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->lock.thieflevel));
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_specproc(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Jm�no spec:");
  gui_define(10,120,20,95,12,0,str_line,wall_procs);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->lock.key_id);
  gui_define(-1,5,37,1,1,0,label,"Parametr:");
  gui_define(20,120,35,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->lock.thieflevel));
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
...
void tma_ifjump(TMULTI_ACTION *q,int linenum,TSTR_LIST ls)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Podm�nka:");
  gui_define(10,10,35,200,12,0,str_line,ls);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(abs(q->twop.parm1)-1);
  gui_define(-1,5,52,1,1,0,label,"��slo ��dku:");
  gui_define(20,120,50,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm2+linenum));
  gui_define(30,5,75,100,30,0,radio_butts,2,"Skok kdy� podm�nka plat�","Skok kdy� podm�nka neplat�");
...
void tma_randjmp(TMULTI_ACTION *q,int linenum)
  ...
  def_dialoge(220,60,410,300,"Macro: If Jump");
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Pravd�podobnost: [%]");
  gui_define(10,120,35,60,12,0,input_line,5,0,100,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm1));
  gui_define(-1,5,52,1,1,0,label,"��slo ��dku:");
  gui_define(20,120,50,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm2+linenum));
...
void tma_ifflagjump(TMULTI_ACTION *q,int linenum)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"��slo vlajky:");
  gui_define(10,120,20,60,12,0,input_line,5,0,255,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(abs(q->twop.parm1)-1));
  gui_define(-1,5,52,1,1,0,label,"��slo ��dku:");
  gui_define(20,120,50,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm2+linenum));
  gui_define(30,5,75,100,30,0,radio_butts,2,"Skok kdy� vlajka nastavena","Skok kdy� vlajka vynulov�na");
  c_default(q->twop.parm1<0);
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
...
void tma_setflag(TMULTI_ACTION *q)
  ...
  gui_define(10,120,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm1));
  gui_define(-1,5,37,1,1,0,label,"Akce:");
  gui_define(20,60,37,80,40,0,radio_butts,3,"Nulovat","Nastavit","Negovat");c_default(q->twop.parm2);
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
void tma_wound(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Min:");
  gui_define(10,120,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->wound.minor));
  gui_define(-1,5,37,1,1,0,label,"Max:");
  gui_define(20,120,35,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->wound.major));
  gui_define(30,120,50,100,10,0,check_box,"Ohroz� skupinu");c_default(q->wound.pflags & 1);
  gui_define(40,120,70,60,80,0,radio_butts,7,"P��m� zran�n�","Hod na obranu","Ohe�","Voda","Zem�","Vzduch","Mysl");
...
void tma_cmoney(TMULTI_ACTION *q)
  ...
  gui_define(-1,5,37,1,1,0,label,"Max:");
  gui_define(20,120,35,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm2));
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,30,80,20,2,button,"Ok");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(CANCEL_BUTT,5,5,80,20,2,button,"Zru�it");gui_on_change(gui_terminate);gui_property(b1,NULL,flat_color(RGB555(15,0,0)),WINCOLOR);
...
static void tma_gomonster(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Odkud:");
  gui_define(10,120,20,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm1));
  gui_define(-1,5,37,1,1,0,label,"Kam:");
  gui_define(20,120,35,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->twop.parm2));
  add_ma_flags(200,220,20,q);
  b1=def_border(1,0);
...
void tma_clock(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,22,1,1,0,label,"Character:");
  gui_define(10,100,20,10,12,0,input_line,2);
  gui_property(b1,NULL,NULL,WINCOLOR);s[0]=q->clock.znak;s[1]=0;set_default(s);
  gui_define(-1,5,37,1,1,0,label,"String:");
  gui_define(20,80,35,100,12,0,input_line,8);strncpy(s,q->clock.string,8);s[8]=0;set_default(s);
  gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(-1,5,52,1,1,0,label,"��slo:");
  gui_define(30,80,50,30,12,0,input_line,8,0,15," %02d");set_default(strs(q->clock.codenum));
...
void tma_text(TMULTI_ACTION *q,char *filename)
  ...
  ls=create_list(10);
  if ((id=load_string_list(&ls,filename)) != 0)
     {
     char s[300];
  ...
  gui_define(-1,5,20,1,1,0,label,"Text:");
  gui_define(10,5,19,170,12,1,str_line,ls);
    c_default(q->text.textindex);
    gui_on_enter(string_list_sup);
    gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  add_ma_flags(20,5,35,q);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,5,60,20,3,button,"Ok");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(CANCEL_BUTT,5,5,60,20,2,button,"Zru�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  redraw_window();
  escape();
  id=o_aktual->id;
  if (id==CANCEL_BUTT)
...
void tma_dialog(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  def_dialoge(420,120,250,290,"Oprav akci");
  gui_define(-1,5,20,1,1,0,label,"Dialog:");
  gui_define(10,5,19,170,12,1,str_line,dlg_names);
    c_default(i);
    gui_on_enter(string_list_sup);
    gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  add_ma_flags(20,5,35,q);
...
void tma_shop(TMULTI_ACTION *q)
  ...
  b1=def_border(5,WINCOLOR);
  def_dialoge(420,120,250,290,"Oprav akci");
  gui_define(-1,5,20,1,1,0,label,"Obchod:");
  gui_define(10,5,19,170,12,1,str_line,ls);
    c_default(i);
    gui_on_enter(string_list_sup);
    gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
...
void tma_create_dropi(TMULTI_ACTION *q,char *name)
  ...
  gui_define(-1,5,20,1,1,0,label,"V�c:");
  gui_define(10,5,19,170,12,1,str_line,ls);
    c_default(it);
    gui_on_enter(string_list_sup);
    gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
...
void tma_unique(TMULTI_ACTION *q,char edit)
  ...
  b1=def_border(1,0);
  gui_define(10,10,19,230,15,1,button,"Oprav vlastnosti predmetu");
    gui_on_change(tma_unique_edit);
    gui_property(b1,NULL,flat_color(0x1f),WINCOLOR);
  gui_define(20,0,0,0,0,0,value_store,4);c_default((int)&q->uniq.item);
  add_ma_flags(30,5,40,q);
  gui_define(OK_BUTT,5,5,60,20,3,button,"Zav��t");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  redraw_window();
...
void tma_globe(TMULTI_ACTION *q)
  ...
  gui_define(-1,5,22,1,1,0,label,"Udalost:");
  b1=def_border(5,WINCOLOR);

  gui_define(5,80,20,200,12,0,str_line,lst);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->globe.event);
  gui_define(6,290,20,60,12,0,input_line,10,-0x7FFFFFFF,0x7FFFFFFF,"%d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->globe.param));
  gui_define(-1,5,52,1,1,0,label,"Sektor:");
  gui_define(10,80,50,60,12,0,input_line,10,0,maplen,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->globe.sector));
  gui_define(-1,5,37,1,1,0,label,"Pri vzniku udalosti poslat akci na:");
  gui_define(-1,5,67,1,1,0,label,"St�na:");
  gui_define(20,80,65,100,12,0,str_line,steny2);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->globe.side);
  gui_define(-1,5,80,100,10,0,label,"P�i volbe sektor 0 severni");
  gui_define(-1,5,90,100,10,0,label,"se akce neposila");
  gui_define(30,20,110,100,10,0,check_box,"Zakaz puvodni chovani");c_default(q->globe.cancel);
...
void tma_sectnumjmp(TMULTI_ACTION *q,int linenum)
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,52,1,1,0,label,"Sektor:");
  gui_define(10,80,50,60,12,0,input_line,6,0,maplen,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->ifsec.sector));
  gui_define(-1,5,67,1,1,0,label,"St�na:");
  gui_define(20,80,65,100,12,0,str_line,steny2);gui_on_enter(string_list_sup);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(q->ifsec.side);
  gui_define(-1,5,82,1,1,0,label,"��slo ��dku:");
  gui_define(30,120,80,60,12,0,input_line,5,0,32767,"%5d");gui_on_exit(test_int);
  gui_property(b1,NULL,NULL,WINCOLOR);set_default(strs(q->ifsec.line+linenum));
  gui_define(40,5,120,120,30,0,radio_butts,2,"Skok kdy� podm�nka plat�","Skok kdy� podm�nka neplat�");
...
void create_multiaction_dialoge()
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(-1,5,20,1,1,0,label,"P��kaz:");
  gui_define(9,10,40,120,206,0,listbox,ls,RGB555(31,31,31),0);gui_on_change(create_multiaction_invalid);
  gui_property(b1,NULL,NULL,WINCOLOR);c_default(-1);
  gui_define(10,136,60,21,167,0,scroll_bar_v,0,10,1,SCROLLBARCOL);
  gui_property(b1,NULL,NULL,WINCOLOR);
  gui_define(11,136,40,21,17,0,scroll_button,-1,0,"\x1e");
  gui_property(b1,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
  gui_define(12,136,230,21,17,0,scroll_button,1,10,"\x1f");
  gui_property(b1,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
  //gui_define(10,5,19,130,12,1,str_line,act_types);c_default(act_default);gui_on_enter(string_list_sup);
  add_ma_flags(20,136,20,&x);
  b1=def_border(1,0);
  gui_define(OK_BUTT,5,5,60,20,3,button,"Vyvo�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(CANCEL_BUTT,5,5,60,20,2,button,"Zru�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  c_set_value(0,9,i);
...
static void copy_and_paste()
{
  ...
  b1=def_border(5,WINCOLOR);
  gui_define(10,10,20,180,20,0,button,"Kop�rovat do pam�ti");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(400,10,70,100,10,0,check_box,"Severn�");c_default(side==0);
  gui_define(410,10,85,100,10,0,check_box,"V�chodn�");c_default(side==1);
  gui_define(420,10,100,100,10,0,check_box,"Ji�n�");c_default(side==2);
  gui_define(430,10,115,100,10,0,check_box,"Z�padn�");c_default(side==3);
  gui_define(440,10,130,180,10,0,check_box,"Tam kde je prim. st�na");c_default(0);
  gui_define(445,10,150,180,12,0,str_line,side_names);gui_property(b1,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  gui_define(450,10,170,180,10,0,check_box,"Tam kde je sec. st�na");c_default(0);
  gui_define(455,10,190,180,12,0,str_line,side_names);gui_property(b1,NULL,NULL,WINCOLOR);c_default(0);
    gui_on_enter(string_list_sup);
  gui_define(-1,10,210,180,12,0,label,"Jak vlo�it?");
  gui_define(460,10,225,180,30,0,radio_butts,3,
                             "P�epsat v�e",
                             "P�epsat pou�it� ud�l.",
                             "P�idat na konec");c_default(1);
  gui_define(20,10,260,180,20,0,button,"Vlo�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  gui_define(30,10,285,180,20,0,button,"Zru�it");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(gui_terminate);
  redraw_window();
  escape();
  id=o_aktual->id;
  if (id==10)
...
void open_multiaction_window()
  ...
  if (find_window(multiaction_win)==NULL)
     ...
     waktual->minsizx=120;
     waktual->minsizy=150;
     gui_define(-1,5,20,1,1,0,label,"Sektor");
     gui_define(-1,5,32,1,1,0,label,"St�na");
     gui_define(-1,5,44,1,1,0,label,"Program:");
     gui_define(-1,3,1,10,10,2,resizer);
     gui_define(10,5,20,60,10,1,input_line,5,0,maplen-1,"%5d");set_default(strs(i));gui_on_exit(test_int);
      gui_property(NULL,NULL,flat_color(0x1f),WINCOLOR);
     gui_define(20,5,32,60,10,1,str_line,steny2);c_default(0);gui_on_enter(string_list_sup);
      gui_property(NULL,NULL,flat_color(0x1f),WINCOLOR);gui_on_change(mult_change_dir);
     create_multiaction_list(&mul_list,i,0);
     gui_define(29,5,55,95,180,0,listbox,mul_list,RGB555(31,31,0),0);c_default(0);
        o_end->autoresizex=1;
        o_end->autoresizey=1;
     gui_define(30,3,66,15,157,1,scroll_bar_v,0,10,1,SCROLLBARCOL);
        gui_property(NULL,NULL,NULL,WINCOLOR);
        o_end->autoresizey=1;
     gui_define(31,3,50,15,15,1,scroll_button,-1,0,"\x1e");
        gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
     gui_define(32,3,60,15,15,2,scroll_button,1,10,"\x1f");
        gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
     gui_define(40,5,5,50,20,3,button,"Vyma�");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(delete_multiaction_ask);
     gui_define(50,60,5,50,20,3,button,"Edit");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(edit_multiaction);
     gui_define(60,5,30,50,20,3,button,"Vlo�");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(create_multiaction_dialoge);
     gui_define(70,60,30,50,20,3,button,"Pam�t");gui_property(b1,NULL,NULL,WINCOLOR);gui_on_change(copy_and_paste);
     //gui_define(70,60,30,50,20,3,button,"Po�ad�");gui_property(b1,NULL,NULL,WINCOLOR);
     redraw_window();
     }
