...
//  if (snd_devnum==DEV_DAC) Sound_StopMixing();
  do
     ...
     if (r==size)
        switch (sect)
         {
         case A_SIDEMAP:
                  map_sides = (TSTENA*)temp;
                  break;
         case A_SECTMAP:
                  map_sectors = (TSECTOR*)temp;
                  break;
         case A_MAPINFO:
                  map_coord = (TMAP_EDIT_INFO*)temp;
         ...
         case A_STRMAIN:
                  ...
                  prepare_graphics(&ofsts,(char*)temp,size,pcx_fade_decomp,SR_GRAFIKA);
                  free(temp);
                  break;
         case A_STRLEFT:
         ...
         case A_STRCEIL:
                  ...
                  prepare_graphics(&ofsts,(char*)temp,size,pcx_15bit_autofade,SR_GRAFIKA);
                  free(temp);
                  break;
         case A_STRFLOOR:
         ...
         case A_MAPMOBS:
                  ...
                  if (mob_template==NULL)
                    ...
                    alock(H_ENEMY);
                    p = (char*)ablock(H_ENEMY);
		    p+=8;
                    h=get_handle_size(H_ENEMY)-8;
                    load_enemies((short*)temp,size,&ofsts,(TMOB *)p,h);
                    aunlock(H_ENEMY);
                    }
                  else
                    {
                    load_enemies((short*)temp,size,&ofsts,mob_template,mob_size);
                    free(mob_template);
         ...
         case A_MAPMACR:
                  ...
                  break;
         case A_MAPVYK:
                  map_vyk = (TVYKLENEK*)temp;
                  vyk_max=size/sizeof(TVYKLENEK);
                  break;
         case A_MOBS:
                  mob_template = (TMOB*)temp;
                  mob_size=size;
                  break;
         case A_MOBSND:
                  snd_load=1;
                  create_sound_table((char*)temp,size);
                  free(temp);
                  break;
		 case A_PASSW :
				  map_with_password=1;
  ...
  while (nmapend);
  fclose(f);
  flag_map=(uint8_t*)getmem(mapsize*4);
