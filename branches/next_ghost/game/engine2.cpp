...
*/
	...
	// TODO: needs testing
	int x, y;
	uint8_t *src = (uint8_t*)zoom.texture, *tmp;
	uint16_t *dst = (uint16_t*)zoom.startptr;
...
void fcdraw(uint16_t *source,uint16_t *target, T_FLOOR_MAP *table) {
	do {
		memcpy(target + table->lineofs/sizeof(uint16_t), source + table->txtrofs/sizeof(uint16_t), table->linesize * sizeof(uint16_t));
	} while ((table++)->counter);
}
