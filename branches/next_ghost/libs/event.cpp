...
T_EVENT_ROOT *add_event_message(T_EVENT_ROOT **tree,int msg)
  ...
  if (*tree==NULL)
     {
     *tree = (T_EVENT_ROOT*)getmem(sizeof(T_EVENT_ROOT));
     r=*tree;
     r->next=NULL;
     }
  else
     {
     r = (T_EVENT_ROOT*)getmem(sizeof(T_EVENT_ROOT));
     r1=*tree;
...
T_EVENT_POINT *add_event(T_EVENT_ROOT **tree,int msg,EV_PROC proc,char end)
  ...
  if (r==NULL)
     ...
     p = (T_EVENT_POINT*)getmem(sizeof(T_EVENT_POINT));
