...
void open_program_window(int track)
  ...
  for(i=0;i<=100;i+=100)
     ...
     gui_define(70+i,32,20+y,50,20,1,button,"+���+");gui_on_change(change_vpoint);
       gui_property(NULL,vga_font,cl2,WINCOLOR);
   ...
   gui_define(230,190,5,80,20,3,button,"St�ed");gui_property(cl,vga_font,&cl2,BUTTONCOLOR);gui_on_change(max_min_mid_align);
   gui_define(240,280,5,80,20,3,button,"Jako doln�");gui_property(cl,vga_font,&cl2,BUTTONCOLOR);gui_on_change(max_min_mid_align);
   gui_define(250,370,5,80,20,3,button,"Jako horn�");gui_property(cl,vga_font,&cl2,BUTTONCOLOR);gui_on_change(max_min_mid_align);
...
void new_edit_sample(int track)
  ...
  gui_define(-1,170,35,1,1,0,label,"Cesta a jm�no zvuku");
  gui_define(-1,170,65,1,1,0,label,"Jm�no stopy (voliteln�)");
  gui_define(-1,170,95,1,1,0,label,"Za��tek opakov�n� (-1 vyp)");
  gui_define(-1,170,125,1,1,0,label,"Konec opakov�n�");
...
static void make()
  ...
  gui_property(def_border(3,WINCOLOR),NULL,flat_color(0x2e0),WINCOLOR);
  gui_define(20,5,5,480,12,3,input_line,100);set_default("Ukl�d�m projekt....");
...
char how_frames(char *prompt,int *value)
  ...
  gui_define(40,90,5,80,20,2,button,"Zru�it");gui_property(&b1,vga_font,NULL,BUTTONCOLOR);gui_on_change(gui_terminate);
  redraw_window();
  chyba:
  escape();
  if (o_aktual->id==30)
...
void control_window()
  ...
  gui_property(NULL,icones,NULL,WINCOLOR);gui_on_change(scroll_support);
  gui_define(20,140,20,65,20,0,button,"Nov�");gui_property(&b1,vga_font,NULL,BUTTONCOLOR);gui_on_change(new_sample);
  gui_define(30,210,20,65,20,0,button,"Odstra�");gui_property(&b1,vga_font,NULL,BUTTONCOLOR);gui_on_change(delete_sample);
  ...
  gui_define(60,140,120,65,20,0,button,"P�ehr�t");gui_property(&b1,vga_font,NULL,BUTTONCOLOR);gui_on_change(prehrat_cele);
  gui_define(70,210,120,65,20,0,button,"N�hled");gui_property(&b1,vga_font,NULL,BUTTONCOLOR);gui_on_change(prehrat_preview);
